...
class RunnerIter(RunnerBase):
    ...
    def train(self):
        ...
        for start_iters in range(
            ...
            if not self.evaluate_only:
                ...
                train_stats = self.train_iters(self.cur_epoch, start_iters)#cur_epoch  当前epoch数 start_iters 当前迭代数
                logging.info(f"Train stats: {train_stats}")  # 检查 train_stats 是否有值
    ...
    def train_iters(self, epoch, start_iters):
        ...
        return self.task.train_iters(
            ...
            start_iters=start_iters,    #当前迭代数
            iters_per_inner_epoch=self.iters_per_inner_epoch,   #每个inner epoch的迭代数
            model=self.model,   #模型
            data_loader=self.train_loader, #训练数据加载器
            optimizer=self.optimizer,   #优化器
            scaler=self.scaler, #混合精度训练
            lr_scheduler=self.lr_scheduler, #学习率调度器
            cuda_enabled=self.cuda_enabled, #是否使用cuda
            log_freq=self.log_freq, #日志打印频率
            accum_grad_iters=self.accum_grad_iters, #梯度累积迭代数
        )
